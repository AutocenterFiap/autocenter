...
class FiltroTokenAcessoTest {
    ...
    void deveAutenticarUsuarioComTokenValido() throws Exception {
        ...
        when(tokenPort.verificarToken("tokenValido")).thenReturn("marcos");
        when(buscarUsuarioComPerfisUseCase.executar("marcos")).thenReturn(usuario);
    ...
    void deveLancarExcecaoUsuarioNaoEncontrado() throws Exception {
        when(request.getHeader("Authorization")).thenReturn("Bearer tokenValido");
        when(tokenPort.verificarToken("tokenValido")).thenReturn("marcos");
        when(buscarUsuarioComPerfisUseCase.executar("marcos"))
                .thenThrow(new UsuarioNaoEncontradoException("O usuário não foi encontrado!"));
    ...
    void deveContinuarFiltroComTokenInvalido() throws Exception {
        ...
        when(tokenPort.verificarToken("tokenInvalido")).thenThrow(new RuntimeException("Token inválido"));
