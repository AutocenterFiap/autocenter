package br.com.autocenterfiap.servico.application.usecase;

import br.com.autocenterfiap.servico.application.dto.ServicoOutput;
import br.com.autocenterfiap.servico.application.port.ServicoRepositoryPort;
import br.com.autocenterfiap.servico.domain.entity.Servico;
import br.com.autocenterfiap.servico.domain.enums.StatusServico;
import br.com.autocenterfiap.servico.domain.exception.ServicoNaoEncontradoException;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;

import java.math.BigDecimal;
import java.util.Optional;

import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.Mockito.*;

@ExtendWith(MockitoExtension.class)
@DisplayName("BuscarServicoPorIdUseCase - Testes Unitários")
class BuscarServicoPorIdUseCaseTest {

    @Mock
    private ServicoRepositoryPort repositoryPort;

    @InjectMocks
    private BuscarServicoPorIdUseCase buscarServicoPorIdUseCase;

    @Test
    @DisplayName("Deve retornar Serviço quando ID existir")
    void deveRetornarServicoQuandoIdExistir() {
        Servico servico = Servico.builder()
                .id(1L)
                .descricao("Troca de óleo")
                .status(StatusServico.ATIVO)
                .valor(BigDecimal.valueOf(100.00))
                .build();

        when(repositoryPort.buscarPorId(1L)).thenReturn(Optional.of(servico));

        ServicoOutput output = buscarServicoPorIdUseCase.executar(1L);

        assertNotNull(output);
        assertEquals(1L, output.getId());
        assertEquals("Troca de óleo", output.getDescricao());
        verify(repositoryPort, times(1)).buscarPorId(1L);
    }

    @Test
    @DisplayName("Deve lançar ServicoNaoEncontradoException quando ID não existir")
    void deveLancarServicoNaoEncontradoExceptionQuandoIdNaoExistir() {
        when(repositoryPort.buscarPorId(999L)).thenReturn(Optional.empty());

        assertThrows(ServicoNaoEncontradoException.class, () -> buscarServicoPorIdUseCase.executar(999L));
        verify(repositoryPort, times(1)).buscarPorId(999L);
    }
}
